...
function input(values: Record<string, string | undefined>) {
  ...
  for (const [key, value] of Object.entries({ name: 'Prepared business', type: 'BARBERSHOP', ...values })) {
...
test('provisioning supports email, phone and both, with normalized login identities', () => {
  for (const values of [
    { email: ' Customer@Example.com ' }, { phone: '050-1234567' },
    ...
    assert.equal(result.value.ownerEmail, values.email ? 'customer@example.com' : '972501234567@phone.torchick.local');
    assert.equal(result.value.phoneIdentity, values.phone ? '972501234567@phone.torchick.local' : null);
  }
});

test('provisioning rejects missing, malformed and reserved identities', () => {
  for (const values of [
    {}, { email: 'bad' }, { phone: 'hello0501234567' }, { phone: '0123' },
...
test('only the explicitly assigned owner phone is an alternate identity, never a public contact', () => {
  const business = { ownerEmail: 'owner@example.com', ownerPhoneIdentity: '972501234567@phone.torchick.local' };
  ...
  assert.equal(isBusinessOwnerIdentity(business.ownerPhoneIdentity, { ownerEmail: business.ownerEmail }), false);
  assert.equal(decideBusinessAdminRoute({ email: business.ownerPhoneIdentity, ...business, isPlatformAdmin: false }), 'owner');
