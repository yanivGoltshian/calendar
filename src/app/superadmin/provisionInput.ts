...
import { isValidEmail, isValidIsraeliMobile, normalizeEmail, normalizePhone } from '@/lib/crypto';
...
const schema = z.object({
  ...
  if (!value.email && !value.phone) ctx.addIssue({ code: 'custom', path: ['email'], message: 'identity' });
  ...
  if (value.phone && (!/^[+\d\s().-]+$/.test(value.phone) || !isValidIsraeliMobile(value.phone))) {
...
export function parseProvisionInput(formData: FormData) {
  const parsed = schema.safeParse(Object.fromEntries(
    ['name', 'type', 'email', 'phone'].map((key) => [key, formData.get(key) ?? '']),
  ));
  ...
  return { ok: true as const, value: {
    name: parsed.data.name, type: parsed.data.type, phone, ownerEmail: email, phoneIdentity,
  } };
}
