  ...
  return (
    <main className="mx-auto max-w-2xl px-4 pb-16 pt-6">
      ...
      <form method="get" className="mb-6 flex flex-wrap items-end gap-2">
        <div>
          <label className="mb-1 block text-sm font-medium text-slate-700">{s.fromLabel}</label>
          <input
            type="date"
            name="from"
            defaultValue={from}
            dir="ltr"
            className="rounded-lg border border-slate-300 px-3 py-2 text-slate-900 outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500"
          />
        </div>
        <div>
          <label className="mb-1 block text-sm font-medium text-slate-700">{s.toLabel}</label>
          <input
            type="date"
            name="to"
            defaultValue={to}
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-3">
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <p className="text-sm text-slate-500">{s.totalAppointments}</p>
          <p className="mt-1 text-2xl font-bold text-slate-900">{summary.totalAppointments}</p>
        </div>
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <p className="text-sm text-slate-500">{s.revenue}</p>
          <p className="mt-1 text-2xl font-bold text-slate-900">
            {formatAgorot(summary.revenueAgorot)}
          </p>
          <p className="mt-0.5 text-xs text-slate-400">{s.revenueHint}</p>
        </div>
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <p className="text-sm text-slate-500">{s.newClients}</p>
          <p className="mt-1 text-2xl font-bold text-slate-900">{summary.newClients}</p>
        </div>
      </div>

      {/* פילוח לפי סטטוס */}
      <section className="mb-6">
        <h2 className="mb-3 text-lg font-bold text-slate-900">{s.byStatusTitle}</h2>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          {summary.byStatus.map((row) => (
            ...
            >
              <p className="text-sm text-slate-500">{t.admin.statuses[row.status]}</p>
      ...
      {/* שירותים מובילים */}
      <section className="mb-6">
        <h2 className="mb-3 text-lg font-bold text-slate-900">{s.topServicesTitle}</h2>
        {summary.topServices.length === 0 ? (
          ...
          <ul className="space-y-2">
            {summary.topServices.map((svc) => (
              ...
              >
                <span className="min-w-0 truncate font-medium text-slate-800">{svc.name}</span>
      ...
      {/* פילוח לפי צוות */}
      <section>
        <h2 className="mb-3 text-lg font-bold text-slate-900">{s.byStaffTitle}</h2>
          ...
          <ul className="space-y-2">
            {summary.byStaff.map((row) => (
              ...
              >
                ...
                <span className="shrink-0 text-sm text-slate-500">{row.count}</span>
              </li>
            ))}
          </ul>
        )}
