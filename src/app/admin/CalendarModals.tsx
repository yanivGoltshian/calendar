...
function Overlay({
  ...
  return (
    <div
      dir="rtl"
      onMouseDown={onClose}
      className="fixed inset-0 z-50 flex items-center justify-center bg-[#1b1715]/40 p-4"
    >
      <div
        onMouseDown={(e) => e.stopPropagation()}
        className="w-full max-w-md rounded-2xl bg-white shadow-xl ring-1 ring-[#e7ddcd]"
      >
        <div className="flex items-center justify-between border-b border-[#efe6d8] px-5 py-3">
          <h2 className="text-lg font-semibold text-[#1b1715]">{title}</h2>
          <button
            type="button"
            onClick={onClose}
            aria-label={c.close}
            className="rounded-lg px-2 py-1 text-[#b3a690] transition hover:bg-[#efe6d8] hover:text-[#4a4038]"
          >
            ✕
          </button>
        </div>
...
export function CreateAppointmentModal({
  ...
  return (
    <Overlay title={c.createTitle} onClose={onClose}>
      <p className="mb-4 text-sm text-[#8f8478]">{staffName}</p>
...
const STATUS_BADGE: Record<string, string> = {
  PENDING: 'bg-[#efe6d8] text-[#4a4038]',
  CONFIRMED: 'bg-brand-100 text-brand-700',
  ARRIVED: 'bg-emerald-100 text-emerald-700',
  CANCELLED: 'bg-red-100 text-red-700',
  NO_SHOW: 'bg-amber-100 text-amber-700',
  DONE: 'bg-[#e7ddcd] text-[#2a2320]',
};
    ...
    <Overlay title={c.detailsTitle} onClose={onClose}>
      ...
      <dl className="space-y-2 text-sm">
        <div className="flex justify-between">
          <dt className="text-[#8f8478]">{t.admin.form.time}</dt>
          <dd dir="ltr" className="font-medium text-[#1b1715]">
            {appt.startLabel}–{appt.endLabel}
          </dd>
        </div>
        <div className="flex justify-between">
          <dt className="text-[#8f8478]">{c.durationLabel}</dt>
          ...
          <dd>
            <span
              className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${
                STATUS_BADGE[status] ?? 'bg-[#efe6d8] text-[#4a4038]'
              }`}
            >
      ...
      <div className="mt-5 border-t border-[#efe6d8] pt-4">
        <p className="mb-2 text-sm font-medium text-[#4a4038]">{c.changeStatus}</p>
