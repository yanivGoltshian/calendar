...
const emptyClass =
  'rounded-xl border border-dashed border-[#d6c8b4] bg-white p-6 text-center text-sm text-[#8f8478]';
  ...
  return (
    <main className="mx-auto max-w-2xl px-4 pb-16 pt-6">
      ...
      <header className="mb-6">
        ...
        <p className="mt-1 text-sm text-[#8f8478]">{t.admin.inventory.subtitle}</p>
      </header>
            ...
            {alerts.map((a) => (
              ...
              >
                <div className="min-w-0">
                  <p className="truncate font-medium text-[#1b1715]">{a.product.name}</p>
                  {a.product.category ? (
                    <p className="text-xs text-[#8f8478]">{a.product.category}</p>
                  ) : null}
                </div>
        ...
        <div className="mb-6 flex flex-wrap gap-2">
          <Link
            ...
            className={`rounded-full px-3 py-1 text-sm ${
              !category ? 'bg-brand-600 text-white' : 'border border-[#d6c8b4] text-[#6e655f]'
            }`}
          >
          ...
          {categories.map((c) => (
            <Link
              ...
              className={`rounded-full px-3 py-1 text-sm ${
                category === c ? 'bg-brand-600 text-white' : 'border border-[#d6c8b4] text-[#6e655f]'
              }`}
            >
              {c}
            </Link>
      ...
      {/* רשימת מוצרים ומלאי */}
      <section>
        <h2 className="mb-3 text-lg font-semibold text-[#1b1715]">{t.admin.inventory.listTitle}</h2>
        {rows.length === 0 ? (
          isSearching ? (
          ...
          <div className="space-y-6">
            {[...groups.entries()].map(([groupLabel, groupRows]) => (
              <div key={groupLabel}>
                <h3 className="mb-2 text-sm font-semibold text-[#8f8478]">{groupLabel}</h3>
                    ...
                    >
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          <p className="truncate font-medium text-[#1b1715]">{row.name}</p>
                          {row.isLow ? (
                      ...
                      <Link
                        ...
                        className="shrink-0 rounded-lg border border-[#d6c8b4] px-3 py-1.5 text-sm text-[#4a4038] hover:bg-[#f7f2ea]"
                      >
        ...
        <>
          ...
          <section className="mt-8">
            ...
            {movements.length === 0 ? (
              ...
              <div className="overflow-x-auto rounded-xl border border-[#e7ddcd] bg-white shadow-sm">
                <table className="w-full text-right text-sm">
                  <thead className="bg-[#f7f2ea] text-xs text-[#8f8478]">
                    <tr>
                  ...
                  <tbody className="divide-y divide-[#efe6d8]">
                    {movements.map((m) => (
                      <tr key={m.id}>
                        <td className="px-3 py-2 font-medium text-[#1b1715]" dir="ltr">
                          {m.quantityDelta > 0 ? `+${m.quantityDelta}` : m.quantityDelta}
                        </td>
                        <td className="px-3 py-2 text-[#6e655f]">
                          {t.admin.inventory.movementTypes[m.type]}
                        </td>
                        <td className="px-3 py-2 text-[#8f8478]">
                          {new Date(m.createdAt).toLocaleDateString('he-IL')}
                        </td>
                        <td className="px-3 py-2 text-[#8f8478]">{m.note ?? ''}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
