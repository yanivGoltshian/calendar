      ...
      <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <dl className="space-y-2 text-sm">
          <div className="flex items-center justify-between gap-3">
            <dt className="text-slate-500">{t.admin.clients.phoneLabel}</dt>
          ...
          {client.email ? (
            <div className="flex items-center justify-between gap-3">
              <dt className="text-slate-500">{t.admin.clients.emailLabel}</dt>
              <dd className="font-medium text-slate-900" dir="ltr">
                {client.email}
              </dd>
            </div>
        ...
        {client.notes ? (
          <div className="mt-3 border-t border-slate-100 pt-3">
            <p className="mb-1 text-sm text-slate-500">{t.admin.clients.notesTitle}</p>
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      {services ? (
                        <p className="mt-0.5 text-sm text-slate-600">{services}</p>
                      ) : null}
                    ...
                    <div className="flex shrink-0 flex-col items-end gap-1">
                      <span
                        className={`rounded-full px-2 py-0.5 text-xs font-medium ${
                          statusClass[appt.status] ?? 'bg-slate-200 text-slate-600'
                        }`}
                      >
                        {statusLabel}
                      </span>
