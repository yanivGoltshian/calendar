...
test('YouTube hero uses private muted looping decorative playback', () => {
  const hv = heroVideoResolve('https://youtu.be/dQw4w9WgXcQ');
  assert.deepEqual(hv, {
    kind: 'embed',
    src: 'https://www.youtube-nocookie.com/embed/dQw4w9WgXcQ?autoplay=1&mute=1&loop=1&controls=0&playsinline=1&rel=0&playlist=dQw4w9WgXcQ',
  });
});
...
test('youtube watch?v= → embed', () => {
  ...
  assert.ok(hv && hv.src.includes('autoplay=1'));
});
...
test('Vimeo hero uses muted looping background playback', () => {
