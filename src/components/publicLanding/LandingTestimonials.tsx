  ...
  return (
    <section id="reviews" className="mx-auto mt-12 max-w-4xl sm:mt-16">
      ...
      {visible.length === 0 && submitHref ? (
        <p className="text-sm text-[color:var(--c-muted,#665d57)]">
          {t.reviews.emptyPublic}
        </p>
      ) : null}
      {visible.length > 0 ? (
        <div
          className={`grid gap-3 sm:grid-cols-2 ${visible.length > 2 ? 'lg:grid-cols-3' : ''}`}
        >
          {visible.map((review, index) => (
            <ReviewCard key={review.id ?? `legacy-${index}`} review={review} />
          ))}
        </div>
      ) : null}
    </section>
