...
assert.ok(
  ...
  'TEST_DATABASE_URL must name a disposable migrated database',
);
...
process.env.FIREBASE_CLIENT_EMAIL =
  'integration@torchick-identity-integration.iam.gserviceaccount.com';
...
const {
  ...
  MAX_BUSINESSES_PER_OWNER,
} =
  require('../src/server/repos/business') as typeof import('../src/server/repos/business');
const { computeTrialHashes } =
  require('../src/server/repos/trialLedger') as typeof import('../src/server/repos/trialLedger');
const { getBusinessAccess } =
  require('../src/server/subscription') as typeof import('../src/server/subscription');
const { findOrCreateClient } =
  require('../src/server/repos/clients') as typeof import('../src/server/repos/clients');
const { getAppointmentsForUser, getUpcomingAppointmentsForUserAtBusiness } =
  require('../src/server/repos/account') as typeof import('../src/server/repos/account');
const { serializeSession } =
  require('../src/lib/session') as typeof import('../src/lib/session');
const { signImpersonationValue } =
  require('../src/server/impersonationToken') as typeof import('../src/server/impersonationToken');
const { getImpersonatedBusinessId } =
  require('../src/server/impersonation') as typeof import('../src/server/impersonation');
const { uploadMedia } =
  require('../src/server/media/upload') as typeof import('../src/server/media/upload');
const { storeBusinessMedia } =
  require('../src/server/media/storage') as typeof import('../src/server/media/storage');
const { saveAllSettingsAction } =
  require('../src/app/admin/settings/actions') as typeof import('../src/app/admin/settings/actions');
const { toggleServiceHiddenAction } =
  require('../src/app/admin/services/actions') as typeof import('../src/app/admin/services/actions');
const { saveStaffAction } =
  require('../src/app/admin/team/actions') as typeof import('../src/app/admin/team/actions');
const { createStaffMember } =
  require('../src/server/repos/staff') as typeof import('../src/server/repos/staff');
const { setSaleLinksAction } =
  require('../src/app/admin/pos/actions') as typeof import('../src/app/admin/pos/actions');
const { POST: subscribe } =
  require('../src/app/api/push/subscribe/route') as typeof import('../src/app/api/push/subscribe/route');
const { GET: returning } =
  require('../src/app/api/public/b/[slug]/returning/route') as typeof import('../src/app/api/public/b/[slug]/returning/route');
const { createOtp } =
  require('../src/server/repos/otp') as typeof import('../src/server/repos/otp');
const { POST: verifyPhoneOtp } =
  require('../src/app/api/otp/verify/route') as typeof import('../src/app/api/otp/verify/route');
const { POST: verifyEmailOtp } =
  require('../src/app/api/otp/email/verify/route') as typeof import('../src/app/api/otp/email/verify/route');
const { POST: verifyFirebasePhone } =
  require('../src/app/api/auth/firebase-phone/route') as typeof import('../src/app/api/auth/firebase-phone/route');
const { provisionBusinessAction, editBusinessDetailsAction } =
  require('../src/app/superadmin/actions') as typeof import('../src/app/superadmin/actions');
const { getBusinessesOwnedByEmail, BusinessIdentityConflictError } =
  require('../src/server/repos/business') as typeof import('../src/server/repos/business');
...
async function inRequest<T>(
  ...
  const implicitTags = {
    tags: [],
    expirationsByCacheHandler: new Map(),
  ...
  return workAsyncStorage.run(work, () =>
    workUnitAsyncStorage.run(store, () => action(request)),
  );
}
...
before(async () => {
  const staffUser = await prisma.user.create({
    data: { phone: `+9725${Date.now().toString().slice(-8)}` },
  });
  const attacker = await prisma.user.create({
    data: { email: outsiderEmail, emailVerified: new Date() },
  });
  const victimUser = await prisma.user.create({
    data: { email: `${prefix}-victim@example.test` },
  });
  ...
  otherBusiness = await prisma.business.create({
    data: {
      slug: `${prefix}-b`,
      name: 'Tenant B',
      ownerEmail: `${prefix}-other@example.test`,
      plan: 'premium',
  ...
  const appointment = await prisma.appointment.create({
    data: {
      ...
      status: 'CONFIRMED',
      totalPriceAgorot: 1000,
      services: {
        create: {
          serviceId,
          nameSnapshot: 'Private treatment',
...
after(async () => {
  ...
  await prisma.trialLedger.deleteMany({
    where: {
      emailHash: {
        in: registrationEmails.map((email) => computeTrialHashes(email, null).emailHash),
      },
    },
  });
...
test('anonymous, client-only and authenticated non-owner cannot resolve a tenant or change settings', async () => {
  for (const cookie of [
    '',
    clientCookie(attackerUserId, outsiderEmail),
    await ownerCookie(outsiderEmail),
  ]) {
    assert.equal(await inRequest(cookie, () => getActiveBusiness()), null);
    const result = await inRequest(cookie, () =>
      saveAllSettingsAction({ ok: false }, settingsForm('Injected')),
    );
    assert.equal(result.ok, false);
    assert.equal(
      (await prisma.business.findUniqueOrThrow({ where: { id: business.id } })).name,
      'Tenant A',
    );
  }
});
...
test('active trial owner can mutate its own tenant but not another tenant resource', async () => {
  ...
  const own = await inRequest(cookie, () =>
    saveAllSettingsAction({ ok: false }, settingsForm('Owner saved')),
  );
  ...
  await inRequest(cookie, () =>
    toggleServiceHiddenAction(form({ id: otherServiceId, hidden: 'true' })),
  );
  assert.equal(
    (await prisma.service.findUniqueOrThrow({ where: { id: otherServiceId } })).hidden,
    false,
  );
  await inRequest(cookie, () =>
    toggleServiceHiddenAction(form({ id: serviceId, hidden: 'true' })),
  );
  assert.equal(
    (await prisma.service.findUniqueOrThrow({ where: { id: serviceId } })).hidden,
    true,
  );
});
...
test('settings rejects new embedded media while preserving unchanged legacy bytes', async () => {
  ...
  try {
    ...
    assert.equal(
      (await inRequest(cookie, () => saveAllSettingsAction({ ok: false }, unchanged))).ok,
      true,
    );
    const preserved = await prisma.business.findUniqueOrThrow({
      where: { id: business.id },
    });
    ...
    for (const [key, value] of [
      ...
      fd.set(key, value);
      assert.equal(
        (await inRequest(cookie, () => saveAllSettingsAction({ ok: false }, fd))).ok,
        false,
      );
      assert.deepEqual(
    ...
    assert.equal(
      (await inRequest(cookie, () => saveAllSettingsAction({ ok: false }, uploaded))).ok,
      true,
    );
  } finally {
    ...
    await prisma.business.update({
      ...
      data: {
        ...
        coverImageUrl: before.coverImageUrl,
...
test('expired and deletion-pending owners cannot mutate or subscribe; explicit recovery lookup stays available', async () => {
  ...
  for (const data of [
    ...
    assert.equal(await inRequest(cookie, () => getActiveBusiness()), null);
    assert.equal(
      (await inRequest(cookie, () => getActiveBusiness({ allowInactive: true })))?.id,
      business.id,
    );
    const result = await inRequest(cookie, () =>
      saveAllSettingsAction({ ok: false }, settingsForm('Suspended write')),
    );
    assert.equal(result.ok, false);
    const response = await inRequest(cookie, () =>
      subscribe(pushRequest(`${prefix}-suspended`)),
    );
  ...
  assert.equal(
    await inRequest(await ownerCookie(outsiderEmail), () =>
      getActiveBusiness({ allowInactive: true }),
    ),
    null,
  );
  await prisma.business.update({
...
test('only an authenticated platform admin with a valid explicit token may impersonate', async () => {
  ...
  assert.equal(
    await inRequest(`${await ownerCookie(outsiderEmail)}; tc_imp=${token}`, () =>
      getActiveBusiness(),
    ),
    null,
  );
  assert.equal(
    await inRequest(await ownerCookie(adminEmail), () => getActiveBusiness()),
    null,
  );
  assert.equal(
    await inRequest(`${await ownerCookie(adminEmail)}; tc_imp=${token}tampered`, () =>
      getActiveBusiness(),
    ),
    null,
  );
  ...
  assert.equal(
    (await inRequest(cookie, () => getActiveBusiness()))?.id,
    otherBusiness.id,
  );
  ...
  await prisma.business.update({
    where: { id: business.id },
    data: { ownerEmail: adminEmail },
  });
  try {
    assert.equal(
      (await inRequest(await ownerCookie(adminEmail), () => getActiveBusiness()))?.id,
      business.id,
    );
...
test('onboarding uploads select the explicitly authorized business and charge only its storage', async () => {
  ...
  await prisma.business.update({
    where: { id: business.id },
    data: { ownerEmail: adminEmail, trialEndsAt: new Date(0) },
  });
  try {
    // Reaching the configuration guard proves the active target, rather than the expired first-owned business, was selected.
    assert.equal(
      (await inRequest(cookie, (request) => uploadMedia(request))).status,
      503,
    );
    assert.equal(
      (await inRequest(await ownerCookie(adminEmail), (request) => uploadMedia(request)))
        .status,
      403,
    );
    ...
    await inRequest(
      `${await ownerCookie(outsiderEmail)}; tc_imp=${signImpersonationValue(otherBusiness.id)}`,
      async () => {
        const authority = await getImpersonatedBusinessId();
    ...
    await prisma.business.update({
      where: { id: business.id },
      data: { ownerEmail, trialEndsAt: business.trialEndsAt },
    });
...
test('push subscriptions require an owner and cannot be stolen across tenants', async () => {
  ...
  assert.equal(
    (await inRequest(owner, () => subscribe(pushRequest(prefix)))).status,
    200,
  );
  assert.equal(
    (await inRequest(owner, () => subscribe(pushRequest(prefix)))).status,
  ...
  assert.equal(
    (await inRequest(other, () => subscribe(pushRequest(prefix)))).status,
    403,
  );
  const stored = await prisma.pushSubscription.findUniqueOrThrow({
    where: { endpoint: `https://fcm.googleapis.com/fcm/send/${prefix}` },
  });
...
test('authenticated push registration rejects arbitrary/loopback destinations but accepts browser push services', async () => {
  ...
  for (const endpoint of [
    ...
    assert.equal(
      (await inRequest(owner, () => subscribe(pushRequest(prefix, endpoint)))).status,
      200,
    );
    assert.equal(
      (await prisma.pushSubscription.findUniqueOrThrow({ where: { endpoint } }))
        .businessId,
      business.id,
    );
  }
...
test('guest contact collision never changes or links an existing client', async () => {
  const beforeVictim = await prisma.client.findUniqueOrThrow({
    where: { id: victim.id },
  });
  const guest = await findOrCreateClient({
    businessId: business.id,
    name: 'Attacker assertion',
    phone: victim.phone,
    email: outsiderEmail,
  });
  ...
  assert.deepEqual(
    await prisma.client.findUniqueOrThrow({ where: { id: victim.id } }),
    beforeVictim,
  );
  assert.deepEqual(
  ...
  assert.deepEqual(
    await getUpcomingAppointmentsForUserAtBusiness(
      {
        userId: attackerUserId,
        phone: victim.phone!,
        email: outsiderEmail,
...
test('authenticated booking links only its fresh client; no guest/legacy contact auto-claim', async () => {
  ...
  const again = await findOrCreateClient({
    ...
    userId: attackerUserId,
    phone: '+972509999999',
    name: 'Changed assertion',
  });
  assert.deepEqual(again, linked);
  assert.deepEqual(await getAppointmentsForUser({ userId: attackerUserId }), {
    upcoming: [],
    past: [],
  });
  await prisma.client.update({
    where: { id: victim.id },
    data: { userId: attackerUserId },
  });
  assert.deepEqual(await getAppointmentsForUser({ userId: attackerUserId }), {
    upcoming: [],
    past: [],
  ...
  await prisma.client.update({
    where: { id: victim.id },
    data: { userId: victimUserId, identityVerifiedAt: new Date() },
  });
...
test('optional transaction rolls back guest contact on booking failure', async () => {
  ...
  await assert.rejects(
    prisma.$transaction(async (tx) => {
      await findOrCreateClient(
        { businessId: business.id, phone: victim.phone, name: marker },
        tx,
      );
  ...
  assert.equal(
    await prisma.client.count({ where: { businessId: business.id, name: marker } }),
    0,
  );
});
...
test('guest separation preserves deny-only blocked-contact policy without reusing the blocked client', async () => {
  ...
  assert.equal(guest.userId, null);
  assert.deepEqual(
    await prisma.client.findUniqueOrThrow({ where: { id: blocked.id } }),
    blocked,
  );
});
...
test('returning endpoint exposes only verified appointments and never treats booked IDs as authority', async () => {
  ...
  const guest = await inRequest(
    '',
    (req) => returning(req, params),
    `/api/public/b/${business.slug}/returning?booked=${victimAppointmentId}`,
  );
  const body = await guest.json();
  assert.deepEqual(body.appointments, []);
  assert.ok(!JSON.stringify(body).includes(victimAppointmentId));
  const owner = await inRequest(clientCookie(victimUserId), (req) =>
...
test('tenant staff editing cannot reassign global verified login identity', async () => {
  ...
  const cookie = await ownerCookie(ownerEmail);
  const denied = await inRequest(cookie, () =>
    saveStaffAction(
      { ok: false, mode: 'edit' },
      form({
        id: staff.id,
  ...
  assert.deepEqual(
    await prisma.user.findUniqueOrThrow({ where: { id: staff.userId! } }),
    beforeUser,
  );
  const allowed = await inRequest(cookie, () =>
    saveStaffAction(
      { ok: false, mode: 'edit' },
      form({
        id: staff.id,
        ...
        name: 'Must not replace global name',
        permissionLevel: 'MANAGER',
        active: 'true',
      }),
    ),
  ...
  assert.equal(
    (await prisma.staffMember.findUniqueOrThrow({ where: { id: staff.id } })).displayName,
    'Local profile',
  );
});
...
test('adding a staff profile cannot overwrite the existing global identity name', async () => {
  await prisma.user.update({
    ...
    data: { phone: '+972506664455', name: 'Verified global name' },
  });
  const beforeUser = await prisma.user.findUniqueOrThrow({ where: { id: victimUserId } });
  const result = await inRequest(await ownerCookie(ownerEmail), () =>
    saveStaffAction(
      { ok: false, mode: 'add' },
      form({
        phone: beforeUser.phone!,
        name: 'Poisoned global name',
  ...
  assert.deepEqual(
    await prisma.user.findUniqueOrThrow({ where: { id: victimUserId } }),
    beforeUser,
  );
});

test('concurrent staff additions return one duplicate result without throwing', async () => {
  const phone = '+972509001103';
  ...
  const results = await Promise.all([
    createStaffMember(business.id, input),
    createStaffMember(business.id, input),
  ]);
  ...
  assert.equal(
    await prisma.staffMember.count({
      where: { businessId: business.id, user: { phone } },
    }),
    1,
  );
});
...
test('only platform admin can provision a customer; creation sends no verification and enters onboarding', async () => {
  ...
  for (const cookie of [
    '',
    await ownerCookie(ownerEmail),
    clientCookie(attackerUserId),
  ]) {
    await assert.rejects(
      inRequest(cookie, () => provisionBusinessAction({}, fd)),
  ...
  assert.ok(
    (
      await inRequest(await ownerCookie(adminEmail), () =>
  ...
  const prepared = await prisma.business.findFirstOrThrow({
  ...
  assert.equal(
    await prisma.otpCode.count({ where: { phone: { in: [email, '+972509003101'] } } }),
    0,
  );
...
test('either verified owner login reaches the same prepared business and client login does not', async () => {
  ...
  for (const identity of [email.toUpperCase(), phoneIdentity]) {
    ...
    assert.deepEqual(
      (await getBusinessesOwnedByEmail(identity)).map((b) => b.id),
      [prepared.id],
    );
    assert.equal(
      (
        await inRequest(cookie, () =>
          saveAllSettingsAction({ ok: false }, settingsForm('Prepared by admin')),
        )
      ).ok,
      true,
    );
    assert.equal(
      (await createBusiness({ ownerEmail: identity, name: 'Stale signup form' })).id,
      prepared.id,
    );
  }
  assert.equal(
    await inRequest(clientCookie(attackerUserId, email, phone), () =>
      getActiveBusiness(),
    ),
    null,
  );
  ...
  await prisma.business.update({
    ...
    data: { phone: '+972509003199' },
  });
  assert.equal(
    (await inRequest(await ownerCookie(phoneIdentity), () => getActiveBusiness()))?.id,
    prepared.id,
  );
  assert.equal(
    await inRequest(await ownerCookie(ownerEmailForPhone('+972509003199')!), () =>
      getActiveBusiness(),
    ),
    null,
  );
  ...
  assert.equal(
    await inRequest(await ownerCookie(phoneIdentity), () => getActiveBusiness()),
    null,
  );
  assert.equal(
    (
      await inRequest(await ownerCookie(phoneIdentity), () =>
        getActiveBusiness({ allowInactive: true }),
      )
    )?.id,
    prepared.id,
...
test('phone-only and email-only provisioning preserve normal owner login and seed existing onboarding', async () => {
  ...
  for (const email of [registrationEmail('provision-email'), phoneIdentity]) {
    const prepared = await createBusiness({
      ownerEmail: email,
      name: `${prefix}-${email === phoneIdentity ? 'phone' : 'email'}`,
      provisioning: {
        adminEmail,
        phoneIdentity: email === phoneIdentity ? phoneIdentity : null,
      },
    });
    const seeded = await prisma.business.findUniqueOrThrow({
      where: { id: prepared.id },
      include: { staff: true, services: true, workingHours: true, settings: true },
    });
    assert.ok(
      seeded.staff.length &&
        seeded.services.length &&
        seeded.workingHours.length &&
        seeded.settings,
    );
    assert.equal(
      (await inRequest(await ownerCookie(email), () => getActiveBusiness()))?.id,
      prepared.id,
    );
  }
});
...
test('concurrent provisioning is idempotent and refuses mismatched existing owner identities', async () => {
  ...
  await assert.rejects(
    createBusiness({ ...input, ownerEmail: otherEmail }),
    BusinessIdentityConflictError,
  );
  await assert.rejects(
    createBusiness({ ...input, ownerEmail }),
    BusinessIdentityConflictError,
  );
  assert.equal(await prisma.user.count({ where: { email: otherEmail } }), 0);
  assert.equal(
    await prisma.business.count({ where: { ownerPhoneIdentity: phoneIdentity } }),
    1,
  );
});
...
test('simultaneous self-signup and provisioning cannot create duplicate ownership', async () => {
  ...
  const outcomes = await Promise.allSettled([
    ...
    createBusiness({
      ownerEmail: email,
      name: `${prefix}-admin-signup`,
      provisioning: { adminEmail, phoneIdentity },
    }),
  ]);
  assert.equal(outcomes[0].status, 'fulfilled');
  if (outcomes[1].status === 'rejected')
    assert.ok(outcomes[1].reason instanceof BusinessIdentityConflictError);
  assert.equal(await prisma.business.count({ where: { ownerEmail: email } }), 1);
});
...
test('an explicit ownership transfer revokes the old provisioned phone without changing other users', async () => {
  ...
  assert.equal(
    await inRequest(await ownerCookie(email), () => getActiveBusiness()),
    null,
  );
  assert.equal(
    (await inRequest(await ownerCookie(replacement), () => getActiveBusiness()))?.id,
    prepared.id,
  );
});
...
test('onboarding atomically seeds an immediately bookable eligible basic trial', async () => {
  ...
  const seeded = await prisma.business.findUniqueOrThrow({
    where: { id: created.id },
    include: { workingHours: true, staff: true, services: true },
  });
  assert.equal(seeded.workingHours.length, 5);
  assert.equal(seeded.staff.length, 1);
  assert.ok(seeded.services.length > 0);
  assert.equal(
    await prisma.serviceStaff.count({ where: { staffId: seeded.staff[0].id } }),
    seeded.services.length,
  );
  const ledger = await prisma.trialLedger.findUniqueOrThrow({
    where: { emailHash: computeTrialHashes(email, null).emailHash },
  ...
  assert.equal(
    (await inRequest(await ownerCookie(email.toUpperCase()), () => getActiveBusiness()))
      ?.id,
    created.id,
  );
  const edited = await inRequest(await ownerCookie(email), () =>
    saveStaffAction(
      { ok: false, mode: 'edit' },
      form({
        id: seeded.staff[0].id,
        phone: '',
        displayName: 'Local owner profile',
        permissionLevel: 'MANAGER',
        active: 'true',
      }),
    ),
  ...
  assert.equal(
    edited.ok,
    true,
    'email-only owner staff can edit tenant-local fields without claiming a phone',
  );
});
...
test('database seed rejection rolls back business, owner and trial allocation together', async () => {
  ...
  try {
    await assert.rejects(
      createBusiness({ ownerEmail: email, ownerName, name: `${prefix}-failed` }),
    );
    assert.equal(await prisma.business.count({ where: { ownerEmail: email } }), 0);
    assert.equal(await prisma.user.count({ where: { email } }), 0);
    assert.equal(
      await prisma.trialLedger.count({
        where: { emailHash: computeTrialHashes(email, null).emailHash },
      }),
      0,
    );
    ...
    await prisma.$executeRawUnsafe(
      `ALTER TABLE "StaffMember" DROP CONSTRAINT "${constraint}"`,
    );
  }
...
test('concurrent registration cannot exceed owner quota or extend the original trial', async () => {
  ...
  const results = await Promise.allSettled(
    Array.from({ length: 6 }, (_, index) =>
      createBusiness({
        ownerEmail: index % 2 ? email.toUpperCase() : email,
        name: `${prefix}-quota-${index}`,
      }),
    ),
  );
  ...
  for (const result of results) {
    if (result.status === 'rejected')
      assert.ok(result.reason instanceof BusinessCreationLimitError);
  }
  assert.equal(
    await prisma.business.count({ where: { ownerEmail: email } }),
    MAX_BUSINESSES_PER_OWNER,
  );
  const ledger = await prisma.trialLedger.findUniqueOrThrow({
    where: { emailHash: computeTrialHashes(email, null).emailHash },
  ...
  for (const result of fulfilled)
    assert.equal(
      result.value.trialEndsAt?.getTime(),
      ledger.originalTrialEndsAt.getTime(),
    );
  ...
  await assert.rejects(
    createBusiness({ ownerEmail: email, name: 'Cannot bypass quota' }),
    BusinessCreationLimitError,
  );
  const restored = await createBusiness({
    ownerEmail: email,
    name: 'Recovery',
    phone: '+972501112233',
  });
...
test('phone OTP stamps phone ownership only after success, including existing unverified staff users', async () => {
  ...
  const unverified = await prisma.user.create({
    data: { phone, name: 'Unverified staff', role: 'STAFF' },
  });
  ...
  assert.equal(
    (await prisma.user.findUniqueOrThrow({ where: { id: unverified.id } }))
      .phoneVerifiedAt,
    null,
  );
  const accepted = await inRequest('', () =>
    verifyPhoneOtp(jsonRequest({ phone, code })),
  );
  ...
  assert.equal(
    (await inRequest('', () => verifyPhoneOtp(jsonRequest({ phone, code })))).status,
    401,
  );
});
...
test('email OTP stamps email verification only after success', async () => {
  ...
  assert.equal(denied.status, 401);
  assert.equal(
    (await prisma.user.findUniqueOrThrow({ where: { id: unverified.id } })).emailVerified,
    null,
  );
  const accepted = await inRequest('', () =>
    verifyEmailOtp(jsonRequest({ email: email.toUpperCase(), code })),
  );
  assert.equal(accepted.status, 200);
  assert.ok(
    (await prisma.user.findUniqueOrThrow({ where: { id: unverified.id } })).emailVerified,
...
test('Firebase verified phone creates a verified user; invalid tokens cannot create one', async () => {
  ...
  try {
    ...
    const accepted = await inRequest('', () =>
      verifyFirebasePhone(jsonRequest({ idToken })),
    );
    assert.equal(accepted.status, 200);
    assert.ok(lookups > 0);
    ...
    await new Promise<void>((resolve, reject) =>
      emulator.close((error) => (error ? reject(error) : resolve())),
    );
  }
});
