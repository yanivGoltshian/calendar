...
## היסטוריית תורים בעמוד העסק

לקוח מזוהה יכול לעבור בין תורים קרובים להיסטוריה בתוך עמוד העסק הקיים, גם כאשר אין
לו תורים עתידיים. ההיסטוריה נטענת בפרטיות במנות של עשרים תורים, ורק עבור שיוך לקוח
מאומת באותו עסק. תור נכלל בהיסטוריה לאחר מועד תחילתו; ביטול עתידי אינו נכלל.

מחיר ההזמנה נלקח מהרישום ההיסטורי. סכום ששולם מוצג רק כאשר קיימת עסקה סגורה יחידה,
תשלומים תואמים וסל המורכב בדיוק משירותי התור, ללא מסמך זיכוי. רישום חסר, מחיר אפס
ישן או שיוך תשלום שאינו חד משמעי מוצגים במפורש כמידע שאינו מתועד. מחיר השירות
הנוכחי אינו משמש להשלמת תשלום עבר. המימוש משתמש בשדות הקיימים ללא שינוי סכימה,
גבייה, החזרים או השלמת נתונים.
...
קישור חיצוני לפרופיל העסק או לביקורות בגוגל ניתן לעריכה בהגדרות העסק ופותח את אתר גוגל.
אשף ההקמה מתמקד בתוכן העמוד וברשתות החברתיות. דילוג על הרשתות ושמירת העמוד משמרים
קישורים והמלצות שכבר נשמרו. תוכן ההמלצות נשמר בנפרד מהקישור; ייבוא ועדכון אוטומטי
של ביקורות אינם זמינים.

כרטיסי ההמלצות מציגים כוכבים רק כאשר נשמר דירוג מספרי שלם בין 1 ל־5.
ייחוס לגוגל נשען על מקור הביקורת השמור. קישור עסקי לבדו אינו מוסיף ייחוס להמלצות.
אפשר להסתיר ביקורת באופן הפיך באמצעות השדה הבא ברשומה השמורה:

```json
{ "hidden": true }
```

ערך שקרי או שדה חסר משאירים את הביקורת גלויה. הסינון מתבצע בתצוגה בלבד,
ושומר את כל הרשומות לצורך עריכה ושחזור. שמירות רגילות בעורך ובהגדרות משמרות את
הביקורות ואת נתוני המקור הקיימים; עריכת המלצה ידנית אינה מקנה נתוני מקור או דירוג מהימנים.
...
- **קטגוריות שירותים**: אפשרות מתקדמת סגורה כברירת מחדל במסך השירותים ובשלב השירותים באונבורדינג. בעל העסק עורך שמות ובוחר את השירותים בכל קטגוריה, עם שיוך יחיד לשירות. כשהאפשרות מופעלת ויש קטגוריות, מתווספות לשוניות בבוחר השירות וברשימה הציבורית, כולל הכול, ללא קטגוריה וקטגוריה ריקה. כיבוי שומר את ההגדרות ומחזיר את התצוגה הרגילה. באונבורדינג אפשר לבחור קטגוריה גם לשירות חדש, והשיוך נשמר יחד עם יצירתו. השמירה מאומתת ומוגנת מדריסה מקבילה. ההרחבה האדיטיבית מוסיפה שדה הגדרות אופציונלי יחיד לעסק, ונדרשת במסלול המיגרציות הקיים לפני פריסת הקוד:

  ```text
  20260917203000_service_categories
  ```
