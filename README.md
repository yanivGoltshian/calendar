# Booqi — פלטפורמת קביעת תורים וניהול עסק

> **הערה על השם:** "Booqi" הוא שם זמני (placeholder). השם הסופי נקבע בנפרד ומרוכז
> בקובץ `src/config/brand.ts` — החלפתו היא שינוי שורה אחת.
